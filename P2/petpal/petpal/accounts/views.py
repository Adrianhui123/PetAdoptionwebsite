    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if user_type == 'seeker':
                ...
                serializer = ShelterUserSerializer(data={'user': user.pk})
                if serializer.is_valid():
                    serializer.save()
                    return Response(data, status=status.HTTP_201_CREATED)
...
class ShelterListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            data = [{'username': shelter.user.username, 'email': shelter.user.email, 'pk': ShelterUserSerializer(shelter).data} for shelter in pet_shelters]
            ...
            if user.username == username:
                try:
                    ...
                    data = {'username': seeker.user.username, 'email': seeker.user.email,
                            'pk': SeekerUserSerializer(seeker).data}
                    return Response(data, status=status.HTTP_200_OK)
